...
import '../../bottom_bar/model/offer_image_model.dart';
import '../../sales_order/model/CustomerDetailModel.dart';
import '../model/get_catefory_brand_list_model.dart';
import '../model/promo_message_model.dart';
...
class CustomerMainController extends GetxController {
  ...
  /// Logged-in customer app: [customerId] from GetCustomerDetails API.
  Future<void> customerDetailForItemsApi() async {
    FormData formData = FormData.fromMap({});
    final data = await GetAPIFunction().apiCall(
    ...
    var responseData = data is String ? jsonDecode(data) : data;
    final model = CustomerDetailResponse.fromJson(responseData);
    if (model.statusCode == 200) {
      final id = model.data?.customerID ?? 0;
      await apiCallGetItem(customerId: id);
    }
  }
